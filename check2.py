...
def generate_ingestion_failed_report(executed_by_name, execution_start_time, execution_end_time):
    ...
    row_pass_fail_na = '<para align="center"><font color="red"><b>Fail</b></font></para>'  # Explicitly set to Fail for failed ingestion

    actual_result_content_parts.append(
        Paragraph(f"Node Execution Status: {ingestion_status}", table_cell_style))
...
WHERE a.workspace_pipeline_id = CAST('{current_pipeline_id}' AS BIGINT)
        ...
        if not ingestion_failed:  # Only add this section if ingestion was NOT failed
            ...
            objective_content = Paragraph(
                f"<link href=\"{pipeline_ui_url}\"><font color=\"blue\">{pipeline_ui_url}</font></link>",
                table_cell_style)
            ...
            test_setup_data = [
                ...
                [Paragraph("<b>Pipeline url</b>", table_header_style), objective_content],
        ...
        for item in steps_data:
            ...
            if item["id"] == 1:  # Ingestion
                ...
                if ingestion_status == "SUCCESS":
                    row_pass_fail_na = '<para align="center"><font color="Green"><b>Pass</b></font></para>'
                elif ingestion_status == "FAILED":
                    row_pass_fail_na = '<para align="center"><font color="red"><b>Fail</b></font></para>'
                ...
                actual_result_content_parts.append(
                    Paragraph(f"Node Execution Status: {overall_execution_status}", table_cell_style))
            ...
            if item["id"] == 2:  # Schema Validation
                if overall_execution_status == "Success":
                    ...
                    if source_schema_is_na or target_schema_is_na:
                        row_pass_fail_na = '<para align="center"><font color="red"><b>Fail</b></font></para>'  # If any schema is NA, it's a Fail
                    elif schema_comments_singular:
                        row_pass_fail_na = '<para align="center"><font color="red"><b>Fail</b></font></para>'  # If singular mismatches, it's a Fail
                    else:
                        row_pass_fail_na = '<para align="center"><font color="Green"><b>Pass</b></font></para>'  # Otherwise, it's a Pass
                ...
                if overall_execution_status == "Success":
                    ...
                    if source_only_count > 0 or destination_only_count > 0:
                        ...
                        row_pass_fail_na = '<para align="center"><font color="red"><b>Fail</b></font></para>'  # If any discrepancies, it's a Fail
                ...
                if overall_execution_status == "Success":
                    ...
                    if comments_raw:
                        ...
                        if has_not_unique:
                            row_pass_fail_na = '<para align="center"><font color="red"><b>Fail</b></font></para>'  # If any comment contains "not_unique", it's a Fail
                        else:
                            row_pass_fail_na = '<para align="center"><font color="Green"><b>Pass</b></font></para>'  # Otherwise, it's a Pass
                    else:
                        actual_result_content_parts.append(Paragraph("N/A", table_cell_style))
                ...
                if overall_execution_status == "Success":
                    ...
                    if comments_raw:
                        ...
                        if has_problematic_nulls:
                            row_pass_fail_na = '<para align="center"><font color="red"><b>Fail</b></font></para>'  # If any comment contains "nulls" (but not "no nulls"), it's a Fail
                        else:
                            row_pass_fail_na = '<para align="center"><font color="Green"><b>Pass</b></font></para>'  # Otherwise, it's a Pass
                    else:
                        actual_result_content_parts.append(Paragraph("N/A", table_cell_style))
...
# Report Directory
NEW_REPORT_DIR = "pipeline_reports_new/pipeline_reports_new"
