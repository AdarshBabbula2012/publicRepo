...
def generate_pipeline_report(spark_session, current_pipeline_id, current_batch_id, current_pipeline_name,
                             ...
                             source_obj_name, target_obj_name, pipeline_status_from_list, pipeline_ui_url):
    """
    ...
    try:
        # Determine if validation details should be processed based on the pipeline's overall status
        # If the pipeline status is FAILED, we assume validation details might not be available or relevant.
        process_validation_details = (pipeline_status_from_list == "SUCCESS")
...
WHERE a.workspace_pipeline_id = CAST('{current_pipeline_id}' AS BIGINT)
        ...
        if spark_session and (
                process_validation_details or current_pipeline_id == "Ingestion pipeline"):  # Added condition for ingestion
            try:
        ...
        objective_content = Paragraph(f"<b>Pipeline URL:</b> <link href=\"{pipeline_ui_url}\">{pipeline_ui_url}</link>",
                                      table_cell_style)
        ...
        test_setup_data = [
            ...
            [Paragraph("<b>Source Object Name</b>", table_header_style), Paragraph(source_obj_name, table_cell_style),
             Paragraph("<b>Target Object Name</b>", table_cell_style), Paragraph(target_obj_name, table_cell_style)],
        ...
        test_setup_table.setStyle(TableStyle([
            ...
            ('BACKGROUND', (0, 5), (0, 5), colors.HexColor("#F3E5F5")),
        ]))
        ...
        for item in steps_data:
            ...
            if item["id"] == 1:  # Ingestion
                overall_execution_status = pipeline_status_from_list
                ...
                actual_result_content_parts.append(
                    Paragraph(f"<b>Pipeline Name:</b> {current_pipeline_name}", table_cell_style))
                actual_result_content_parts.append(Spacer(1, 2))

                # Ingestion status logic
                if overall_execution_status == "SUCCESS":
                    row_pass_fail_na = "Pass"
                elif overall_execution_status == "FAILED":
            ...
            if item["id"] == 2:  # Schema Validation
                if overall_execution_status == "Success":
                    ...
                    nested_schema_table = Table(schema_details_table_data, colWidths=[None])  # None means auto-width
                ...
                if overall_execution_status == "Success":
                    ...
                    all_discrepancies = []  # To store all discrepancies for CSV

                    for comment in comments_raw:
                        parts = comment.split('||', 1)
                        comment_type = parts[0]
                        formatted_data_content = ""
                        raw_data_content = ""  # Store raw content for CSV
                        if len(parts) > 1:
                            json_or_string_part = parts[1]
                            raw_data_content = json_or_string_part  # Store raw content
                            try:
                                ...
                                formatted_data_content = re.sub(r'\|\|[0-9a-fA-F]{64}$', '',
                                                                json_or_string_part).strip()

                        if "present_only_in_source" in comment_type:
                            source_only_count += 1
                            source_only_details.append(formatted_data_content)
                            all_discrepancies.append({"type": "present_only_in_source", "data": raw_data_content})
                            ...
                            all_discrepancies.append({"type": "present_only_in_destination", "data": raw_data_content})
                        ...
                        if len(comments_raw) <= 10:
                            if source_only_details:
                                ...
                                for detail in source_only_details:
                                    ...
                                    actual_result_content_parts.append(Spacer(1, 2))  # Added spacer
                            ...
                            # Generate CSV file for discrepancies
                            csv_filename = f"data_validation_discrepancies_{current_pipeline_id}_{current_batch_id}.csv"
                            # Ensure NEW_REPORT_DIR is accessible and exists
                            if not os.path.exists(NEW_REPORT_DIR):
                                os.makedirs(NEW_REPORT_DIR)
                            data_validation_csv_path = os.path.join(NEW_REPORT_DIR, csv_filename)

                            try:
                                with open(data_validation_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
                                    fieldnames = ['discrepancy_type', 'data_content']
                                    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                                    writer.writeheader()
                                    for disc in all_discrepancies:
                                        writer.writerow(
                                            {'discrepancy_type': disc['type'], 'data_content': disc['data']})
                                logging.info(f"CSV file generated at: {data_validation_csv_path}")
                            except Exception as e:
                                logging.error(f"Error writing CSV file {data_validation_csv_path}: {e}")
                                data_validation_csv_path = "Error generating CSV"  # Indicate error in path
                            ...
                            actual_result_content_parts.append(Spacer(1, 6))
                            actual_result_content_parts.append(
                                Paragraph(f"<i>Full list in: {data_validation_csv_path}</i>", table_cell_style))
                ...
                if overall_execution_status == "Success":
                    ...
                    if comments_raw:
                        ...
                        for comment in comments_raw:
                            if "nulls" in comment.lower() and "no nulls" not in comment.lower():
        ...
        main_table.setStyle(TableStyle([
            ...
            # Span the first 3 columns for "Execution Start Date/Time" header
            ('SPAN', (0, 2), (2, 2)),
            # Span the last 2 columns for "Execution End Date/Time" header
            ('SPAN', (3, 2), (4, 2)),

            # MODIFIED: For Row 3 (Execution Start/End Date/Time values)
            # Span the first 3 columns for start time value
            ('SPAN', (0, 3), (2, 3)),
            # Span the last 2 columns for end time value
            ('SPAN', (3, 3), (4, 3)),
            ...
            ('VALIGN', (0, 5), (-1, -1), 'TOP'),  # Vertical align top for body rows (starts from new index 5)
        ]))

        story_elements.append(main_table)
        story_elements.append(Spacer(1, 20))

        # This section is removed as per user's request for CSV output
        # if detailed_data_validation_comments_for_section:
        #     story_elements.append(PageBreak())
        #     story_elements.append(Paragraph("<b>Detailed Data Validation Logs</b>", detailed_log_heading_style))
        #     for comment_line in detailed_data_validation_comments_for_section:
        #         story_elements.append(Paragraph(comment_line, detailed_log_content_style))
        #     story_elements.append(Spacer(1, 20))
...
# Hardcoded values for overall workflow context
executed_by = exec_row['cru_by']
execution_start_dt = exec_row['valid_from_ts']
execution_end_dt = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
for idx, pipeline in enumerate(pipelines_to_process):
    ...
    if p_id != "N/A":
        ...
        try:
            spark_session = spark  # ensure spark_session is available here
            workspace_df = spark_session.read.format("jdbc").options(
                url=PIPELINE_DB_URL,
    ...
    if workspace_id != "N/A" and p_id != "N/A":
        ui_url = 'https://testnabu-3.modak.com:8125'
    ...
    try:
        spark_session = spark  # ensure spark_session is available here
        source_target_df = spark_session.read.format("jdbc").options(
            url=PIPELINE_DB_URL,
        ...
        if source_target_row:
            ...
            logging.info(
                f"Successfully loaded Source: {source_obj_name}, Target: {target_obj_name} for pipeline {p_id}.")
        else:
    ...
    current_pipeline_story = generate_pipeline_report(
...
combined_pdf_path = os.path.join(NEW_REPORT_DIR, "Combined_Test_Execution_Report.pdf")
